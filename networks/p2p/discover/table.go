...
func newTable(cfg *Config) (Discovery, error) {
	...
	tab := &Table{
		net:         cfg.udp,
		db:          db,
		self:        NewNode(cfg.Id, cfg.Addr.IP, uint16(cfg.Addr.Port), []uint16{uint16(cfg.Addr.Port)}, cfg.NodeType),
...
func (tab *Table) pingpong(w *bondproc, pinged bool, id NodeID, addr *net.UDPAddr, tcpPort uint16, nType NodeType) {
	...
	w.n = NewNode(id, addr.IP, uint16(addr.Port), []uint16{tcpPort}, nType)
