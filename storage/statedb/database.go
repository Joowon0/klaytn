...
var (
	...
	memcacheFreecacheEvauateGauge    = metrics.NewRegisteredGauge("trie/freecache/evacuate", nil)
	memcacheFreecacheExpiredGauge    = metrics.NewRegisteredGauge("trie/freecache/expired", nil)
	memcacheFreecacheEntryGauge      = metrics.NewRegisteredGauge("trie/freecache/entry", nil)
	memcacheFreecacheAccessTimeGauge = metrics.NewRegisteredGauge("trie/freecache/access/time", nil)
	memcacheFreecacheHitCountGauge   = metrics.NewRegisteredGauge("trie/freecache/hit/count", nil)
	memcacheFreecacheMissCountGauge  = metrics.NewRegisteredGauge("trie/freecache/miss/count", nil)
	memcacheFreecacheLookupGauge     = metrics.NewRegisteredGauge("trie/freecache/lookup", nil)
	memcacheFreecacheHitRateGauge    = metrics.NewRegisteredGauge("trie/freecache/hit/rate", nil)
	memcacheFreecacheOverwriteGauge  = metrics.NewRegisteredGauge("trie/freecache/overwrite", nil)
...
type Database struct {
	...
	trieNodeCache *freecache.Cache // GC friendly memory cache of trie node RLPs
...
func NewDatabaseWithCache(diskDB database.DBManager, cacheSizeMB int, daBlockNum uint64) *Database {
	var trieNodeCache *freecache.Cache
	if cacheSizeMB > 0 {
		cacheSizeByte := cacheSizeMB * 1024 * 1024
		trieNodeCache = freecache.NewCache(cacheSizeByte)
		logger.Info("Initialize fastCache", "maxBytes", cacheSizeByte)
	}
	return &Database{
		diskDB:                diskDB,
...
func (db *Database) getCachedNode(hash common.Hash) []byte {
	if db.trieNodeCache != nil {
		if enc, err := db.trieNodeCache.Get(hash[:]); err == nil && enc != nil {
...
func (db *Database) UpdateMetricNodes() {
	...
	if db.trieNodeCache != nil {
		memcacheFreecacheEvauateGauge.Update(db.trieNodeCache.EvacuateCount())
		memcacheFreecacheExpiredGauge.Update(db.trieNodeCache.ExpiredCount())
		memcacheFreecacheEntryGauge.Update(db.trieNodeCache.EntryCount())
		memcacheFreecacheAccessTimeGauge.Update(db.trieNodeCache.AverageAccessTime())
		memcacheFreecacheHitCountGauge.Update(db.trieNodeCache.HitCount())
		memcacheFreecacheMissCountGauge.Update(db.trieNodeCache.MissCount())
		memcacheFreecacheLookupGauge.Update(db.trieNodeCache.LookupCount())
		memcacheFreecacheHitRateGauge.Update(int64(db.trieNodeCache.HitRate() * 100))
		memcacheFreecacheOverwriteGauge.Update(db.trieNodeCache.OverwriteCount())
	}
}
